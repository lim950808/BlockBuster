		...
		success:function(data){
			...
			if(data.length > 0){
				for(var datum of data){
					cartList += "<div name='product"+datum.id+"'><div class='row'><input type='hidden' value='"+datum.id+"' class='no' name='id'>";
					cartList += "<input type='hidden' value='"+datum.productId+"' id='pno' name='pno'>";
					...
					cartList += "<a href='/Product/"+datum.pno+"' style='color:black;'>"+datum.productNm+"</a></strong></h5>";
		...
		success:function(data){
			...
			for(var datum of data){
				if(datum.saleId>0){
					var price=0;
					if(datum.unitCd=='01'){
						price = (datum.price - datum.amount)*datum.count;
					}else if(datum.unitCd='02'){
						price = (datum.price*(1-(datum.amount/100)))*datum.count;
					}
				}else{
					price = datum.price*datum.count;
				}
				if(price < 0){
					price = 0;
				}
...
$(".cnt").change(function(){
	...
	var count = $(".count:eq(" + n + ")").val();
	var id = $(".no:eq(" + n + ")").val();
	var dataList={"count":count,"id":id};
	$.ajax({
		async:false,
		url : "/Cart/update",
		type : "post",
